...
\end{document}
  ...
  \textbf{Bold} % This command bolds the text.
	...
	\begin{frame}[fragile]
		\frametitle{Setting your own custom commands}
		
		Using \verb|\newcommand{cmd}{def}| in your preamble, you can define your own commands, for anything from easy shortcuts to larger stuff.\pause\biggap
		
		For example, here's a command this set of slides uses to create big gaps in-between paragraphs:
		\begin{lstlisting}
\newcommand{\biggap}{\newline{}\newline{}}
		\end{lstlisting}\pause
		
		Your new command can also take in data that gets used by it:
		\begin{lstlisting}
\newcommand{command}[number_of_arguments]{definition}
		\end{lstlisting}
...
\end{enumerate}
	...
	\section{Math Mode, Special Characters and Verbatim Environments}
...
\end{math}
	...
	\begin{frame}[fragile]
		...
		Notice the different styles of text throughout this presentation for certain words and characters? \pause \verb|Like|
		\begin{lstlisting}
			this?
		\end{lstlisting}\pause
		Sometimes, you want to print certain characters and words \textit{as} they are without any of \LaTeX{}'s formatting. \pause To do that \textit{inline}, use the {\verb|\verb|}$||$ command (note the vertical bars we use instead of curly braces here; use math mode to type in the bars). For \textit{dedicated} environments, use \verb|\begin{verbatim}| and remember to end it. \pause
		Verbatim environments are also useful for code samples, for which we need a dedicated \verb|lstlisting| environment from the \verb|listings| package (which you create just as you'd create documents, dedicated math environments et cetera). \pause Syntax highlighting for most programming languages is supported here (i.e. \verb|\begin{lstlisting}[language=Python]|), with extensive configuration and customisability options for languages not supported out-of-the-box.
	\end{frame}
	...
	\begin{frame}[fragile]
		...
		See those citations on Wikipedia articles? \pause You can do the same thing in \LaTeX{} using Bib\TeX{}! \pause
		...
		Types of Bib\TeX{} entries include \verb|article| (for journals), \verb|book|, \verb|manual| and \verb|inproceedings| (for conference proceedings). There's also a catch-all \verb|misc| type (used in the next slide) for entries that don't quite fit into the standard categories.
	\end{frame}
  ...
  howpublished = {\url{https://www.example.com}}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item Did the \LaTeX{} compiler format them nicely?\pause
			\item What changed about \textit{how} you wrote, now that you were writing a research ``article" (well, \textit{paragraph(s)})?\pause
			\item Were there any \textit{quirks} in the formatting?\pause
			\item Any compilation warnings and/or errors?\pause
			\item What did you write about? You don't have to say if you don't want to.\pause
		\end{itemize}
	...
	\begin{frame}[fragile]
		...
		Beamer is a document class meant specifically for making slideshows, much like the ones you'd see on Microsoft PowerPoint and/or LibreOffice Impress (only with less animations and transitions, given that your \LaTeX{} slides will typically compile to PDF). \pause\biggap
		
		In a new \verb|.tex| file, set the document class to \verb|beamer| at the very beginning of your preamble, then set the title, author and year as you did before (I'll show you later how to make a Beamer-specific table of contents). \pause Then, add the following to your document (within \verb|\begin{document}| and \verb|\end{document}|): \verb|\frame{\titlepage}|. This will make a title slide for you. \pause
		
		You might notice it looks a bit \textit{plain}. \pause That's why you can use one of Beamer's built-in \textit{themes} to add a bit more style to your slides. \pause I use the ``Madrid" theme, like so (in the preamble, after the document class declaration): \verb|\usetheme{Madrid}|
	\end{frame}
	
	\begin{frame}[fragile]
		\frametitle{How Beamer Works}
		Within your singular document, Beamer works with multiple ``frames", each of which connote 1 slide, including any pauses in-between (if there are any). \pause The slide count at the bottom refers to the number of \textit{frames}, while the number of \textit{overall} pages for the compiled PDF will consist of every frame and every pause in-between.\pause\biggap
		
		As I've demonstrated here, figures, math mode, verbatim environments and links can easily be embedded into your slide, and you'd do that just as you would in an article (for verbatim environments, start your frame like so: \verb|\begin{frame}[fragile]|). \pause You can even make citations and bibliographies on Beamer slides, though that's rarely done in practice and I'm not showing you how to do that here. \pause\biggap
		
		To add a pause to your Beamer frame couldn't be any simpler\pause{:} \verb|\pause|
	\end{frame}
	
	\begin{frame}[fragile]
		\frametitle{Your first Beamer slide (well, \textit{frame})}
		Set up your frame as described in the previous slide, then within that frame, put in:
		\begin{lstlisting}
\frametitle{My first frame}
This is text in a frame! \pause
I just paused my slide! \pause
Did you know you could use \verb|\href{URL}{text}|
to embed hyperlinks into text? \pause
\href{https://www.example.com}{Like this!}
		\end{lstlisting}
		Don't forget to start your frame with \verb|\begin{frame}[fragile]| so you don't get compilation errors when trying to use \verb|\verb|!\newline{}
	...
	\begin{frame}[fragile]
		\frametitle{Tables of Contents in Beamer}
		Sections in your slideshow can be defined just as you would do in articles, books et cetera.
		\begin{lstlisting}
	\section{title}
		\end{lstlisting}
		To create a table of contents based on those sections, add \textit{this} to your preamble (replace \verb|frim| with \verb|frame|; I had compilation errors with \verb|frame| for this slide): 
		\begin{lstlisting}
\AtBeginSection[]
{
	\begin{frim}
	...
	\begin{frame}[fragile]
		\frametitle{Final exercise for the day: Your first Beamer presentation}
		Time for one more exercise before we end this workshop!\pause\biggap
		
		Create a short slideshow with \verb|beamer| on a topic of your choice!\pause\newline{}
		
		Have at least one frame other than the title slide, but other than that, the slideshow shouldn't be \textit{too} long.\pause\newline{}
		
		Include at least \textbf{one} figure containing an image.\pause\newline{}
		\textit{Optionally}, include at least one formula.
	\end{frame}
	
	\begin{frame}
		\frametitle{Feedback on Beamer presentation exercise}
		So$\ldots$ how did you get on?
		\begin{itemize}
			\item How did you find making your presentation in \LaTeX{} compared to PowerPoint or Impress et cetera?\pause
			\item Did the \LaTeX{} compiler format your slides nicely?\pause
			\item What changed about \textit{how} you wrote the content in each slide (if anything changed at all)?\pause
			\item Were there any \textit{quirks} in the formatting?\pause
			\item Any compilation warnings and/or errors?\pause
			\item What did you write about? You don't have to say if you don't want to.\pause
		\end{itemize}
	...
	\begin{frame}
		...
		\begin{itemize}
			\item<1-> \TeX{} StackExchange forum
			\item<2-> Overleaf's \textit{own} \LaTeX{} tutorials (they're how \textbf{I} learnt \LaTeX{} back then)
			\item<3-> r/\LaTeX{} Reddit forum
			\item<4-> \href{https://www.tablesgenerator.com/}{Tables Generator for \LaTeX{}}
			\item<5-> \href{https://en.wikibooks.org/wiki/LaTeX/Mathematics\#List_of_mathematical_symbols}{Wikibooks section listing all of the mathematical symbols you can use in \LaTeX{}, so that you don't have to remember them all.}
			\item<6-> \href{https://tug.ctan.org/info/symbols/comprehensive/symbols-a4.pdf}{The Comprehensive \LaTeX{} Symbols List}
		\end{itemize}
	\end{frame}
	
	\begin{frame}
		\frametitle{Thanks for attending this workshop!}
		I am on:
		\begin{itemize}
			\item \href{https://www.linkedin.com/in/zishan-rahman-6b0769210/}{LinkedIn}
			\item \href{https://mastodon.social/@zishanrahman18}{Mastodon}
			\item \href{https://bsky.app/@zishanrahman18}{BlueSky}
			\item \href{https://www.instagram.com/zishanrahman18/}{Instagram}
			\item \href{https://github.com/Zishan-Rahman}{GitHub} (\href{https://github.com/Zishan-Rahman/GetTeXnicalStuff}{I put the slides up there as well})
			\item \href{https://zishan-rahman.github.io/}{My website} (including blog and project portfolio, currently working on updating it!)
		\end{itemize}
		Advice for you on \LaTeX{}:
		\begin{itemize}
			\item Practice, practice, practice
			\item Try experimenting with different types of \LaTeX{} documents
			\item Seek assistance for compilation errors and more about about specific \LaTeX{} and Bib\TeX{} problems and information.
			\item Never give up!
		\end{itemize}
	\end{frame}
\end{document}